...
const previewParam = "?_ab=0&_fd=0&_sc=1&preview_theme_id=140265619770";

test.beforeAll(() => {
  const ssPath = "./test-data/render-screenshots";
  readdir(ssPath, (err, files) => {
    if (err) throw err;
    for (const file of files) {
      rmSync(ssPath + "/" + file, { recursive: true, force: true });
    }
  });
});
    ...
    test(`Check ${url}`, async ({ page }, testInfo) => {
      ...
      if (
        ...
        for (const { width, height, size } of viewports) {
          ...
          await page.goto(url + previewParam);
